...
public class DAO extends DBContext {
    ...
    public ArrayList<Slot> getAllSlot(String lecture) {
        if (lecture == null) {
            lecture = "";
        }
        ArrayList<Slot> slist = new ArrayList<>();
        String sql = "  select * from Slot s, [Group] g, Room r, Instructor i, [Subject] sub\n"
                + "  where s.GroupID = g.GroupID and r.RoomID =s.RoomID and i.InstructorID = g.InstructorID and sub.SubID =g.SubID and i.UserName = ? ";
        try {
            PreparedStatement stm = connection.prepareStatement(sql);
    ...
    public void UpdateAttendance(int sid, int stid, int checkAttend) {
        String sql = "update [CheckAttedance] set [CheckAttendance] =? , [RecordTime] = GETDATE() where [SlotID] =? and [SID] = ?";
        try {
            PreparedStatement stm = connection.prepareStatement(sql);
            stm.setInt(1, checkAttend);
            stm.setInt(2, sid);
            stm.setInt(3, stid);
            stm.executeUpdate();
        } catch (Exception e) {
    ...
    public static void main(String[] args) {
        DAO dao = new DAO();
        System.out.println(dao.getAllSlot("sonnt"));
    }
}
